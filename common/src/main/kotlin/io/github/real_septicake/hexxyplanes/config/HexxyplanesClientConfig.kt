package io.github.real_septicake.hexxyplanes.config
...
object HexxyplanesClientConfig {
    @JvmStatic
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexxyplanesServerConfig.holder.config = HexxyplanesServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexxyplanesServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hexxyplanes.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HexxyplanesServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexxyplanesServerConfig.ServerConfig()
    }
